...
class threadsafe_queue
{
	private:
		...
		mutable atomic_mutex mut;
		std::queue<T> data_queue;
		std::condition_variable_any EmptyCond, FullCond;
	public:
		...
		{
			std::lock_guard<atomic_mutex> first_mut(other.mut);
		...
		bool full() const
		{
			...
			std::lock_guard<atomic_mutex> block(mut);
		...
		{
			std::unique_lock<atomic_mutex> pop_mut(mut);
		...
		{
			std::unique_lock<atomic_mutex> push_mut(mut);
